...
const COMMUNITY_TENURE_DAYS = 7;
const COMMUNITY_SCAM_MIN_VOTES = 5;
const COMMUNITY_SCAM_MIN_RATIO = 0.8;
const COMMUNITY_MIN_SPREAD_HOURS = 24;
...
function computeVerdict({ safeBrowsing, urlscan, heuristics, community }) {
...
async function checkDomain(domain) {
  ...
  const [safeBrowsing, urlscan, community] = await Promise.all([
    checkGoogleSafeBrowsing(domain),
    checkUrlscan(domain),
  ...
  const verdict = computeVerdict({ safeBrowsing, urlscan, heuristics, community });
  const result = {
    domain,
    verdict,
    ...
    checks: { safeBrowsing, urlscan, heuristics, community },
